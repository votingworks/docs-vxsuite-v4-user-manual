...
VotingWorks applications provide additional system security by allowing you to confirm the software is the same as the certified version. The feature is available on all machines for all authenticated users. On VxAdmin, VxCentralScan, and VxPrint, the `Signed Hash Validation` button is in the "Settings" menu. On VxScan, VxMark, and VxMarkScan, the `Signed Hash Validation` button is on the main menu for each user type. Within the VxScan election manager menu, navigate to the "More" tab. After selecting the button, the machine will generate the hash and display it in the form of a QR code.
